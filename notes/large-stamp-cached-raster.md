...
5. **Dark band / square overlap artifact** (≥256px live stroke, SrcOver): UI batches were SrcOver-compounding onto the layer across time-slices. Fix: `StrokeMaskCached` — persistent per-tile max-alpha mask for the stroke, layer dirty tiles rebuilt each batch as `SrcOver(beforeTile, brush, mask)` using `DirectDrawOutput.BeforeTiles`. Fallbacks: `CachedLightenScratch`, `CachedTileMajorLighten`. Sub-256px / non-live keep direct `CachedTileMajor`.
