...
public sealed class DiscordRichPresenceService : IDisposable
{
    ...
    public void Start()
    {
        ...
        try
        {
            ...
            // If the underlying library starts throwing inside its event pump,
            // immediately tear down the client so it cannot crash the host.
            _client.OnError += (_, _) => SafeDisable();
    ...
    private void PushPresence(string details, string state)
    {
        ...
        {
            // If Discord RPC starts throwing (e.g. due to malformed Assets coming
            // back from the IPC layer), disable the integration so it cannot take
    ...
    private void SafeDisable()
    {
        ...
        {
            Stop();
            App.Config.DiscordRichPresenceEnabled = false;
            App.Config.Save();
        }
        catch
        {
            // Swallow any config write issues; the goal is to keep the app alive.
        }
    }
