...
public sealed class ToolPreset
{
    ...
    // Legacy JSON fields — read-only for migration, never written.
    [JsonInclude]
    [JsonPropertyName("engine")]
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWriting)]
    private ToolPresetEngine? _legacyEngine;

    [JsonInclude]
    [JsonPropertyName("inputProcess")]
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWriting)]
    private InputProcessType? _legacyInputProcess;

    [JsonInclude]
    [JsonPropertyName("outputProcess")]
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWriting)]
    private OutputProcessType? _legacyOutputProcess;

    /// <summary>Assistant factory type id when <see cref="Kind"/> is <see cref="ToolKind.Assistant"/>.</summary>
    public string? AssistantType { get; set; }

    public PerspectiveAssistantMode AssistantPerspectiveMode { get; set; } = PerspectiveAssistantMode.TwoPoint;

    public bool AssistantFisheyeEnabled { get; set; }

    public double AssistantFovDegrees { get; set; } = 180;

    public int AssistantGridSubdivisions { get; set; } = 4;

    public bool AssistantSnapEnabled { get; set; } = true;

    /// <summary>When true (default), new rulers attach to the active editing layer; otherwise a host layer is created at the stack bottom.</summary>
    public bool AssistantCreateAtEditingLayer { get; set; } = true;
    ...
    public void EnsureDefaultBrushOverride(BrushPresetOverrideDocument? factoryFallback = null)
    {
        ...
        if (!Kind.IsBrushFamily()) return;

        DefaultBrushOverride = BrushOverride?.DeepClone()
            ?? factoryFallback?.DeepClone()
            ?? (Kind == ToolKind.Smudge ? ToolGroupConfig.CreateFactorySmudgeOverride().DeepClone() : null);
    }
    ...
    public void SyncSelectModeFromKind()
    {
        if (Kind != ToolKind.Select) return;
        // SelectMode is authoritative for selection tools.
    }
    ...
    public SelectableObjectFlags SelectableObjectFlags { get; set; } = SelectableObjectFlags.Ruler;
    public ObjectSelectionMode ObjectSelectionMode { get; set; } = ObjectSelectionMode.Replace;

    /// <summary>Migrate legacy engine / input+output JSON to <see cref="Kind"/>. Call after deserialization.</summary>
    public void MigrateFromLegacyProcesses()
    {
        if (Kind != default)
        {
            ClearLegacyProcessFields();
            return;
        }

        if (_legacyEngine is { } engine)
        {
            Kind = ToolProcessMapping.FromLegacyEngine(engine, this);
            if (engine == ToolPresetEngine.Select)
                ToolProcessMapping.SyncModesFromLegacyInput(this, ToolProcessMapping.GetInput(this));
        }
        else if (_legacyInputProcess is { } input && _legacyOutputProcess is { } output)
        {
            Kind = ToolProcessMapping.FromLegacyProcesses(input, output);
            ToolProcessMapping.SyncModesFromLegacyInput(this, input);
        }
        else
        {
            Kind = ToolKind.Brush;
    ...
    private void ClearLegacyProcessFields()
    {
        _legacyEngine = null;
        _legacyInputProcess = null;
        _legacyOutputProcess = null;
    }
}
...
public sealed class ToolGroup
{
    ...
    public ToolKind DefaultKind { get; set; } = ToolKind.Brush;

    [JsonInclude]
    [JsonPropertyName("defaultEngine")]
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWriting)]
    private ToolPresetEngine? _legacyDefaultKind;
    ...
    public string ActiveIcon =>
        CustomIcon ?? ActivePreset?.PresetIcon ?? Icons.DefaultIcon(DefaultKind);

    [JsonIgnore]
    public ToolPreset? ActivePreset =>
        Presets.FirstOrDefault(p => p.Id == LastActivePresetId) ?? Presets.FirstOrDefault();

    internal void MigrateFromLegacyProcesses()
    {
        if (DefaultKind == default && _legacyDefaultKind is { } engine)
            DefaultKind = ToolProcessMapping.FromLegacyEngine(engine, new ToolPreset());
        _legacyDefaultKind = null;
    }
}
...
public sealed class ToolGroupConfig
{
    ...
    {
        EnsureAssistantToolGroup();
        EnsureObjectToolPreset();

        foreach (var group in Groups)
        {
            group.MigrateFromLegacyProcesses();
            ...
            {
                var defaultPreset = new ToolPreset { Name = group.Name, Kind = group.DefaultKind };
                defaultPreset.MigrateFromLegacyProcesses();
    ...
    public ToolGroup CreateGroupFromDefault(ToolGroup template, IReadOnlyList<BrushAsset> assets)
    {
        var group = new ToolGroup
        {
            ...
            DefaultKind = template.DefaultKind,
    ...
    {
        if (group.DefaultKind == ToolKind.Smudge)
        {
            var template = Defaults().First(g => g.DefaultKind == ToolKind.Smudge);
            foreach (var src in template.Presets)
                group.Presets.Add(ClonePreset(src));
            return;
        }

        var eraserGroup = group.DefaultKind == ToolKind.Eraser;
        foreach (var asset in assets)
        {
            ...
            var preset = new ToolPreset
            {
                ...
                Kind = eraserGroup ? ToolKind.Eraser : ToolKind.Brush,
                BrushId = asset.Id,
    ...
    private void EnsureObjectToolPreset()
    {
        var operation = Groups.FirstOrDefault(g => g.DefaultKind == ToolKind.MoveLayer);
        if (operation == null)
            return;

        if (operation.Presets.Any(p => p.Id == ObjectPresetId || p.Kind == ToolKind.Object))
            return;

        operation.Presets.Insert(0, new ToolPreset
        {
            Id = ObjectPresetId,
            Name = "Object",
            Kind = ToolKind.Object,
            SelectableObjectFlags = SelectableObjectFlags.Ruler,
            ObjectSelectionMode = ObjectSelectionMode.Replace,
            PresetIcon = Icons.ShapeOutline,
        });
    }

    private void EnsureAssistantToolGroup()
    {
        var template = Defaults().First(g => g.DefaultKind == ToolKind.Assistant);
        var group = Groups.FirstOrDefault(g => g.DefaultKind == ToolKind.Assistant);
        if (group == null)
        {
            Groups.Add(CloneTemplateGroup(template));
            return;
        }

        EnsureBuiltinAssistantPresets(group, template);
    }

    private static void EnsureBuiltinAssistantPresets(ToolGroup group, ToolGroup template)
    {
        MigrateLegacyFisheyePreset(group);

        var existingIds = group.Presets.Select(p => p.Id).ToHashSet(StringComparer.Ordinal);
        foreach (var src in template.Presets)
        {
            if (src.Id == null || existingIds.Contains(src.Id))
                continue;

            var json = JsonSerializer.Serialize(src, JsonOpts);
            var preset = JsonSerializer.Deserialize<ToolPreset>(json, JsonOpts)!;
            preset.Id = src.Id;
            group.Presets.Add(preset);
        }
    ...
    private static void MigrateLegacyFisheyePreset(ToolGroup group)
    {
        foreach (var preset in group.Presets.Where(p => p.AssistantType == PaintingAssistant.FisheyeType))
        {
            preset.AssistantType = PaintingAssistant.PerspectiveType;
            preset.AssistantFisheyeEnabled = true;
        }

        if (group.Presets.Any(p => p.Id == AssistantPerspectivePresetId))
            group.Presets.RemoveAll(p => p.Id == AssistantFisheyePresetId);
        else if (group.Presets.FirstOrDefault(p => p.Id == AssistantFisheyePresetId) is { } legacy)
            legacy.Id = AssistantPerspectivePresetId;
    }

    private static ToolGroup CloneTemplateGroup(ToolGroup template)
    {
        var group = new ToolGroup
        {
            Id = Guid.NewGuid().ToString("N")[..8],
    ...
    private static bool IsAssetBackedGroup(ToolGroup group)
        => group.DefaultKind is ToolKind.Brush or ToolKind.Eraser or ToolKind.Smudge;
    ...
    public void SyncWithAssets(IReadOnlyList<BrushAsset> assets, ToolGroup? preferredGroup = null)
    {
        ...
        var brushGroup = (preferredGroup?.DefaultKind == ToolKind.Brush ? preferredGroup : null)
                       ?? Groups.FirstOrDefault(g => g.DefaultKind == ToolKind.Brush)
                       ?? Groups.First();

        var eraserGroup = Groups.FirstOrDefault(g => g.DefaultKind == ToolKind.Eraser);
        ...
        foreach (var group in Groups)
        {
            ...
            group.Presets.RemoveAll(p => p.BrushId == null && p.Kind.IsBrushFamily());
        }
    ...
    public static string DefaultIcon(ToolKind kind) => Icons.DefaultIcon(kind);
    ...
    internal const string ObjectPresetId = "builtin-object";
    internal const string AssistantRulerPresetId = "builtin-assistant-ruler";
    internal const string AssistantPerspectivePresetId = "builtin-assistant-perspective";
    internal const string AssistantFisheyePresetId = "builtin-assistant-fisheye";

    // ── Factory defaults ──────────────────────────────────────────────────────

    internal static List<ToolGroup> Defaults() =>
    [
        WithDefaultCategory(new() { Name = "Brush",  DefaultKind = ToolKind.Brush,  Shortcut = new(Key.B),
            Presets = [], Categories = [] }),
        WithDefaultCategory(new() { Name = "Eraser", DefaultKind = ToolKind.Eraser, Shortcut = new(Key.E),
            Presets = [], Categories = [] }),
        WithDefaultCategory(CreateDefaultSmudgeGroup()),
        WithDefaultCategory(new()
        {
            Name = "Select", DefaultKind = ToolKind.Select, Shortcut = new(Key.S),
            Presets =
            [
                new() { Name = "Rectangle", Kind = ToolKind.Select, SelectMode = SelectMode.Rect },
                new() { Name = "Lasso",     Kind = ToolKind.Select, SelectMode = SelectMode.Lasso, Stabilization = 0.3 },
                new() { Name = "Polygon",   Kind = ToolKind.Select, SelectMode = SelectMode.PolylineLasso },
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "Magic Wand", DefaultKind = ToolKind.MagicWand, Shortcut = new(Key.W),
            Presets = [new() { Name = "Magic Wand", Kind = ToolKind.MagicWand }]
        }),
        WithDefaultCategory(new()
        {
            Name = "Fill", DefaultKind = ToolKind.Fill, Shortcut = new(Key.G),
            Presets = [
                new() { Name = "Fill", Kind = ToolKind.Fill },
                new() { Name = "Lasso Fill", Kind = ToolKind.LassoFill, Stabilization = 0.3, Antialiasing = true }]
        }),

        WithDefaultCategory(new()
        {
            Name = "Operation", DefaultKind = ToolKind.MoveLayer, Shortcut = new(Key.V),
            Presets =
            [
                new()
                {
                ...
                new() { Id = MoveLayerPresetId, Name = "Move Layer", Kind = ToolKind.MoveLayer },
                new() { Id = SelectLayerPresetId, Name = "Select Layer", Kind = ToolKind.SelectLayer }
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "View", DefaultKind = ToolKind.Hand, CustomIcon = Icons.Hand,
            Presets =
            [
                new() { Id = ViewHandPresetId,    Name = "Hand",     Kind = ToolKind.Hand },
                new() { Id = ViewRotatePresetId,  Name = "Rotate",   Kind = ToolKind.Rotate },
                new() { Id = ViewZoomInPresetId,  Name = "Zoom In",  Kind = ToolKind.Zoom, ZoomDirection = 1 },
                new() { Id = ViewZoomOutPresetId, Name = "Zoom Out", Kind = ToolKind.Zoom, ZoomDirection = -1 },
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "Eyedropper", DefaultKind = ToolKind.Eyedropper, Shortcut = new(Key.I),
            Presets = [new() { Id = EyedropperPresetId, Name = "Eyedropper", Kind = ToolKind.Eyedropper }]
        }),
        WithDefaultCategory(new()
        {
            Name = "Gradient", DefaultKind = ToolKind.Gradient,
            Presets =
            [
                new() { Name = "Linear", Kind = ToolKind.Gradient, GradientType = GradientType.Linear },
                new() { Name = "Radial", Kind = ToolKind.Gradient, GradientType = GradientType.Radial },
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "Shape", DefaultKind = ToolKind.Shape,
            Presets =
            [
                new() { Name = "Rectangle", Kind = ToolKind.Shape, ShapeKind = ShapeKind.Rectangle, PolylineClosePath = true },
                new() { Name = "Ellipse",   Kind = ToolKind.Shape, ShapeKind = ShapeKind.Ellipse,   PolylineClosePath = true },
                new() { Name = "Line",      Kind = ToolKind.Shape, ShapeKind = ShapeKind.Line,      ShapeDrawMode = ShapeDrawMode.Stroke },
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "Polyline", DefaultKind = ToolKind.Polyline,
            Presets =
            [
                new() { Name = "PolyLine", Kind = ToolKind.Polyline, PolylineClosePath = false },
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "Liquify", DefaultKind = ToolKind.Liquify,
            Presets =
            [
                new() { Name = "Liquify", Kind = ToolKind.Liquify, LiquifyMode = LiquifyMode.Push, LiquifySize = 80, LiquifyStrength = 0.3 },
            ]
        }),
        WithDefaultCategory(new()
        {
            Name = "Assistants", DefaultKind = ToolKind.Assistant, Shortcut = new(Key.U),
            CustomIcon = Icons.LineVariant,
            Presets =
            [
                new()
                {
                    Id = AssistantRulerPresetId,
                    Name = "Line",
                    Kind = ToolKind.Assistant,
                    AssistantType = PaintingAssistant.RulerType,
                    PresetIcon = Icons.LineVariant,
                },
                new()
                {
                    Id = AssistantPerspectivePresetId,
                    Name = "Perspective",
                    Kind = ToolKind.Assistant,
                    AssistantType = PaintingAssistant.PerspectiveType,
                    AssistantPerspectiveMode = PerspectiveAssistantMode.TwoPoint,
                    AssistantGridSubdivisions = 6,
                    PresetIcon = Icons.RectangleOutline,
                },
            ]
        }),
    ];
