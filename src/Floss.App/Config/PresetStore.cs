...
public sealed class PresetStore
{
        ...
        using (var command = connection.CreateCommand())
        {
            ...
            while (reader.Read())
            {
                groups.Add(new ToolGroup
                {
                    ...
                    DefaultKind = ParseToolKind(reader.GetString(3)),
    ...
    public void SaveToolGroups(IReadOnlyList<ToolGroup> groups)
    {
        ...
        for (var groupIndex = 0; groupIndex < groups.Count; groupIndex++)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                Add(command, "$engine", group.DefaultKind.ToString());
            ...
            for (var presetIndex = 0; presetIndex < group.Presets.Count; presetIndex++)
            {
                ...
                Add(command, "$engine", preset.Kind.ToString());
                Add(command, "$input", "");
                Add(command, "$output", "");
    ...
    private static ToolKind ParseToolKind(string value)
    {
        if (Enum.TryParse<ToolKind>(value, out var kind))
            return kind;
        if (Enum.TryParse<ToolPresetEngine>(value, out var legacy))
            return ToolProcessMapping.FromLegacyEngine(legacy, new ToolPreset());
