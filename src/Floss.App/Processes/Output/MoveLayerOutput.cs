...
public sealed class MoveLayerOutput : IOutputProcess
{
    ...
    /// original offset. Matches Drawpile's DP_ops_move_rect: select → erase
    ...
    private void ShiftPixels(DrawingLayer layer, int dx, int dy, ToolContext ctx)
    {
        ...
        // Capture content pixels (Drawpile: DP_layer_content_select).
        var captured = layer.Pixels.Capture(contentBounds);

        // Erase old position FIRST (Drawpile: fill_rect REPLACE+transparent).
