...
{
    private static readonly string[] SidebarItems = ["General", "Keyboard & Gestures", "Plugins"];
    ...
    private Control BuildShell()
    {
        var sidebar = new Border
        {
            ...
            Child = new StackPanel
            {
                ...
                Children = { _sidebarBtns[0], _sidebarBtns[1], _sidebarBtns[2] }
            }
        };
    ...
    {
        var content = new StackPanel { Spacing = 0, Margin = new Thickness(20, 16, 20, 20) };
        content.Children.Add(PageHeader("Plugins"));
        content.Children.Add(new TextBlock
        {
            Text = "Enable or disable plugins installed in your Floss plugins folder. Restart Floss for changes to take effect.",
            FontSize = 10,
            Foreground = new SolidColorBrush(Color.Parse(TextMuted)),
        ...
        var plugins = PluginCatalog.Discover();
        if (plugins.Count == 0)
        {
            content.Children.Add(new TextBlock
            {
                Text = $"No plugins found in {AppPaths.PluginsDirectory}",
                FontSize = 11,
                Foreground = new SolidColorBrush(Color.Parse(TextMuted)),
                TextWrapping = TextWrapping.Wrap,
            });
        ...
        else
        {
            foreach (var plugin in plugins)
                content.Children.Add(PluginRow(plugin));
        }
    ...
    private static Control PluginRow(PluginDescriptor plugin)
    {
        ...
        check.IsCheckedChanged += (_, _) => PluginCatalog.SetEnabled(plugin.Id, check.IsChecked ?? false);
        ...
        {
            Text = plugin.SkipReason ?? System.IO.Path.GetFileName(plugin.DllPath),
            FontSize = 9,
            Foreground = new SolidColorBrush(Color.Parse(TextMuted)),
        };
        var labels = new StackPanel { Spacing = 1, Children = { name, detail } };

        var row = new DockPanel { LastChildFill = true, Margin = new Thickness(8, 6) };
