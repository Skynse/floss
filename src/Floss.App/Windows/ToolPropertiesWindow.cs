...
public sealed class ToolPropertiesWindow : Window
{
        ...
        Action? onOpenNodeGraphEditor = null)
    {
        ...
        _isBrushTool = toolPreset.Kind.IsBrushFamily();
    ...
    private string[] BuildCategories()
    {
        ...
        return _toolPreset.Kind switch
        {
            ToolKind.Fill => ["Fill Settings", "Paint Settings"],
            ToolKind.LassoFill => ["Lasso Fill Settings", "Paint Settings", "Lasso Input"],
            ToolKind.Select => ["Selection Settings", "Lasso Input"],
            ToolKind.Gradient => ["Gradient Settings", "Paint Settings"],
            ToolKind.Shape => ["Stroke Settings", "Paint Settings"],
            ToolKind.MagicWand => ["Magic Wand Settings"],
            ToolKind.Liquify => ["Liquify Settings"],
            ToolKind.Eyedropper => ["Eyedropper Settings"],
            ToolKind.MoveLayer => ["Tool Info"],
            _ => ["Properties"]
        };
    }
    ...
    private Control BuildGenericCategoryContent(int categoryIndex)
    {
        ...
        var output = _toolPreset.Kind;
        var isPaint = output is ToolKind.Brush
            or ToolKind.LassoFill
            or ToolKind.Fill
            or ToolKind.Gradient
            or ToolKind.Shape;

        var cat = _categories[categoryIndex];

        if (_toolPreset.Kind == ToolKind.Assistant && cat == "Assistant Settings")
        {
            var assistantType = _toolPreset.AssistantType ?? PaintingAssistant.RulerType;
            panel.Children.Add(BuildGenericToggleRow("Create at editing layer",
                _toolPreset.AssistantCreateAtEditingLayer,
                v => CommitTool(p => p.AssistantCreateAtEditingLayer = v),
                toolPropId: "assistant.createAtEditingLayer"));
            panel.Children.Add(BuildGenericToggleRow("Snap",
                _toolPreset.AssistantSnapEnabled, v => CommitTool(p => p.AssistantSnapEnabled = v), toolPropId: "assistant.snap"));

            if (assistantType is PaintingAssistant.PerspectiveType or PaintingAssistant.FisheyeType)
            {
                panel.Children.Add(BuildGenericComboRow<PerspectiveAssistantMode>("Perspective",
                    _toolPreset.AssistantPerspectiveMode,
                    v => CommitTool(p => p.AssistantPerspectiveMode = v),
                    toolPropId: "assistant.perspectiveMode"));
                panel.Children.Add(BuildGenericToggleRow("Fisheye",
                    _toolPreset.AssistantFisheyeEnabled,
                    v => CommitTool(p => p.AssistantFisheyeEnabled = v),
                    toolPropId: "assistant.fisheye"));
                if (_toolPreset.AssistantFisheyeEnabled)
                {
                    panel.Children.Add(BuildGenericSliderRow("FOV", 10, 360,
                        _toolPreset.AssistantFovDegrees,
                        v => CommitTool(p => p.AssistantFovDegrees = v),
                        "°", step: 1, toolPropId: "assistant.fov"));
                }

                panel.Children.Add(BuildGenericSliderRow("Grid", 2, 12,
                    _toolPreset.AssistantGridSubdivisions,
                    v => CommitTool(p => p.AssistantGridSubdivisions = (int)Math.Round(v)),
                    "", step: 1, toolPropId: "assistant.grid"));
            }
        ...
        switch (output)
        {
            case ToolKind.Fill when cat == "Fill Settings":
            ...
            case ToolKind.LassoFill when cat == "Lasso Fill Settings":
                panel.Children.Add(BuildGenericSliderRow("Tolerance", 0, 1.0,
            ...
            case ToolKind.LassoFill when cat == "Lasso Input":
            case ToolKind.Select when cat == "Lasso Input":
            ...
            case ToolKind.Select when cat == "Selection Settings":
                panel.Children.Add(BuildGenericComboRow<SelectMode>("Selection Mode",
                    _toolPreset.SelectMode, v => CommitTool(p => { p.SelectMode = v; p.SyncSelectModeFromKind(); }), toolPropId: "select.mode"));
            ...
            case ToolKind.Gradient when cat == "Gradient Settings":
            ...
            case ToolKind.Shape when cat == "Stroke Settings":
            ...
            case ToolKind.MagicWand when cat == "Magic Wand Settings":
                panel.Children.Add(BuildGenericSliderRow("Tolerance", 0, 1.0,
            ...
            case ToolKind.Liquify when cat == "Liquify Settings":
            ...
            case ToolKind.Eyedropper when cat == "Eyedropper Settings":
            ...
            case ToolKind.MoveLayer when cat == "Tool Info":
                panel.Children.Add(new TextBlock
                {
    ...
    {
        var isBrushTool = preset.Kind.IsBrushFamily();
        if (isBrushTool != _isBrushTool) return false;
        if (_isBrushTool) return true;
        return preset.Kind == _toolPreset.Kind
            && preset.Kind == _toolPreset.Kind;
    }
