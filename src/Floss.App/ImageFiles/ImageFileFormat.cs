...
public static class DocumentRasterizer
{
    ...
    /// <summary>Rasterize a single layer (or group subtree) on a transparent background.</summary>
    public static unsafe SKBitmap RenderLayerBitmap(DrawingDocument document, DrawingLayer layer)
    {
        ArgumentNullException.ThrowIfNull(layer);

        var width = Math.Max(1, document.Width);
        var height = Math.Max(1, document.Height);
        var bgra = new LayerCompositor().CompositeToBgra([layer], width, height);
