...
public static class KraImporter
{
    ...
    public static DrawingDocument Load(Stream stream)
    {
        ...
        var imageName = imageElement.Attribute("name")?.Value ?? "Imported Document";
        ...
        int depth)
    {
        ...
        layer.IsClipping = KraChannelFlags.HasInheritAlpha(element.Attribute("channelflags")?.Value)
            || element.Attribute("clippingmask")?.Value == "1"
            || element.Attribute("clipping")?.Value == "1";
        layer.IsAlphaLocked = KraChannelFlags.HasAlphaLock(element.Attribute("channellockflags")?.Value);
    ...
    private static void ValidateMimeType(ZipArchive archive)
    {
        ...
        {
            throw new InvalidDataException("File is not a valid KRA document.");
        }
    }
