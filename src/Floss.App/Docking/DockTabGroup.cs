...
public sealed class DockTabGroup : Grid
{
    ...
    /// <summary>Tab strip control — used for drag-drop hit testing.</summary>
    public Control TabStrip => _tabStrip;

    /// <summary>Each tab's bounds in coordinates relative to <paramref name="relativeTo"/>.</summary>
    public IReadOnlyList<(string PanelId, Rect Bounds)> GetTabBoundsRelativeTo(Visual relativeTo)
    {
        var result = new List<(string, Rect)>(_panelIds.Count);
        foreach (var id in _panelIds)
        {
            if (!_tabs.TryGetValue(id, out var tab))
                continue;
            var matrix = tab.TransformToVisual(relativeTo);
            if (matrix == null)
                continue;
            ...
            if (bounds.Width <= 0 || bounds.Height <= 0)
                continue;
            result.Add((id, new Rect(topLeft, bounds.Size)));
        }
