...
public partial class MainWindow
{
    ...
    private IReadOnlyList<int> EffectiveLayerSelection()
        => GetLayersPanel().SelectedLayerIndices.Count > 1
            ? (IReadOnlyList<int>)GetLayersPanel().SelectedLayerIndices.OrderBy(x => x).ToList()
            : new[] { _canvas.ActiveLayerIndex };

    private string LayerSelectionLabel()
    {
        var n = GetLayersPanel().SelectedLayerIndices.Count > 1 ? GetLayersPanel().SelectedLayerIndices.Count : 1;
        ...
        PreviewHandle? previewHandle = null)
    {
        ...
        PixelRegion CapturedPreviewRegion()
        {
            var union = PixelRegion.Empty;
            foreach (var (idx, (captureRegion, _)) in captured)
            {
                if (idx >= 0 && idx < _canvas.Layers.Count)
                    union = union.Union(captureRegion);
            }
            return union.ClipTo(_canvas.Document.Width, _canvas.Document.Height);
        }

        PixelRegion FilterPreviewDirtyRegion()
        {
            var region = CapturedPreviewRegion();
            foreach (var idx in layers)
            {
                if (idx >= 0 && idx < _canvas.Layers.Count)
        ...
        void ApplyPreviewNow()
        {
            ...
            using (_canvas.Document.RenderLock.Write())
            {
        ...
        void RevertPreview()
        {
            ...
            if (captured.Count > 0)
            {
                ...
                {
                    _canvas.Document.NotifyChanged(dirty, layers.Count == 1 ? layers[0] : null);
                    _canvas.InvalidateCompositorUnclipped(dirty, layers.Count == 1 ? layers[0] : null);
                }
            }
        }
    ...
    private Task ShowMessage(string title, string message) => MessageDialog.ShowAsync(this, title, message);
}
