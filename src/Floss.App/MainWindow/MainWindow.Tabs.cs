...
public partial class MainWindow
{
    ...
    private void SwitchToTab(DocumentTab tab)
    {
        ...
        ResetTransientInputState(fullRecover: true);
        ...
        if (_activeTab != null)
        {
            ...
            foreach (var i in GetLayersPanel().SelectedLayerIndices)
        ...
        foreach (var i in tab.SelectedLayerIndices)
            GetLayersPanel().AddLayerSelection(i);
        if (GetLayersPanel().LayerSelectionCount == 0 && _canvas.Layers.Count > 0)
    ...
    private async System.Threading.Tasks.Task CloseTabAsync(DocumentTab tab)
    {
        ...
        if (_tabs.Count == 0)
        {
            ...
            if (_canvasHost != null)
            {
                for (var i = _canvasHost.Children.Count - 1; i >= 0; i--)
                {
                    if (_canvasHost.Children[i] is DrawingCanvas)
                        _canvasHost.Children.RemoveAt(i);
                }
            }
            else if (ReferenceEquals(_canvasFrame?.Child, tab.Canvas))
            {
                _canvasFrame.Child = null;
            }
    ...
    {
        WireFeatureSessionDocumentEvents();
        ...
        _canvas.AssistantsChanged += OnCanvasAssistantsChanged;
        _canvas.ActiveLayerChanged += OnCanvasActiveLayerChanged;
        ...
        _canvas.ActivatePresetById = presetId => TryActivatePresetById(presetId);
    ...
    {
        UnwireFeatureSessionDocumentEvents();
        _canvas.StatsChanged -= OnCanvasStatsChanged;
        _canvas.HistoryChanged -= OnCanvasHistoryChanged;
        _canvas.SelectionChanged -= OnCanvasSelectionChanged;
        _canvas.LayersChanged -= OnCanvasLayersChanged;
        ...
        _canvas.CursorPreviewChanged -= OnCanvasCursorPreviewChanged;
        _canvas.SmartShapeUiChanged -= UpdateSmartShapeLauncher;
        _canvas.ActivatePresetById = null;
    }
    ...
    private void OnCanvasAssistantsChanged(object? s, EventArgs e)
    {
        InvalidateLayerListCache();
        ScheduleLayerListRebuild();
    }

    private void OnCanvasActiveLayerChanged(object? s, EventArgs e)
    {
        GetLayersPanel().RefreshLayerRowSelectionStyles();
        Avalonia.Threading.Dispatcher.UIThread.Post(UpdateStatus, Avalonia.Threading.DispatcherPriority.Background);
        UpdateSelectionActionBar();
    }
