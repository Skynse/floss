...
public partial class MainWindow : ICanvasInputHost
{
    ...
    private LayerPickerPopup? _layerPickerPopup;
    ...
    private void WireNodeGraphKeyboardSurface()
        => GetNodeGraphPanel().WireKeyboardSurface();
    ...
    void ICanvasInputHost.StartLayerPickDrag(Point canvasPosition)
        => _canvas.StartLayerPickDrag(canvasPosition);

    void ICanvasInputHost.UpdateLayerPickDrag(Point canvasPosition)
        => _canvas.UpdateLayerPickDrag(canvasPosition);

    void ICanvasInputHost.CompleteLayerPickDrag(Point viewportPosition, Point canvasPosition)
    {
        var found = _canvas.CompleteLayerPickDrag(canvasPosition);
        if (found.Count == 0 || _canvas.Document == null || _workspaceViewport == null)
            return;

        _layerPickerPopup?.Close();
        var screen = _workspaceViewport.PointToScreen(viewportPosition);
        var workArea = Screens.ScreenFromPoint(screen)?.WorkingArea
            ?? Screens.Primary?.WorkingArea
            ?? new PixelRect(screen, new PixelSize(800, 600));

        var doc = _canvas.Document;
        _layerPickerPopup = new LayerPickerPopup(
            doc,
            found,
            _canvas.ActiveLayerIndex,
            index =>
            {
                _canvas.SelectLayer(index);
                GetLayersPanel().ApplySingleLayerSelection(index);
            },
            indices => ExpandAndScrollToLayers(indices));
        _layerPickerPopup.Closed += (_, _) => _layerPickerPopup = null;
        _layerPickerPopup.ShowNear(screen, workArea);
    }

    void ICanvasInputHost.CancelLayerPickDrag()
        => _canvas.CancelLayerPickDrag();
    ...
    private void OnKeyDownTunnel(object? sender, KeyEventArgs e)
    {
        ...
        {
            if (GetNodeGraphPanel().Editor?.TryHandleKeyDown(e) == true)
                e.Handled = true;
            return;
        }
    ...
    private void OnKeyUpTunnel(object? sender, KeyEventArgs e)
    {
        ...
        if (_keyboardInputScope.ShouldRouteToNodeGraph(focused))
        {
            if (GetNodeGraphPanel().Editor?.TryHandleKeyUp(e) == true)
                e.Handled = true;
            return;
        }
    ...
    /// <param name="fullRecover">
    /// When true, also runs <see cref="DrawingCanvas.RecoverInputState"/> (tab switch,
    /// window deactivate). Chrome focus changes only need the input router reset.
    /// </param>
    private void ResetTransientInputState(bool fullRecover = false)
    {
        if (_canvas == null)
            return;
    ...
    private double GetActiveToolSize()
    {
        ...
        if (preset == null)
            return GetBrushPanel().SizeSlider.Value;

        return preset.Kind switch
        {
            ToolKind.Liquify => preset.LiquifySize,
            _ when preset.Kind.IsBrushFamily() => _activePreset?.Size ?? _canvas.Brush.Size,
            _ => GetBrushPanel().SizeSlider.Value
        };
    }
    ...
    {
        var preset = _activeToolGroup?.ActivePreset;
        return preset?.Kind == ToolKind.Liquify ? 10 : GetBrushPanel().SizeSlider.Minimum;
    }
    ...
    {
        var preset = _activeToolGroup?.ActivePreset;
        if (preset?.Kind == ToolKind.Liquify)
            return 500;
    ...
    internal void SyncBrushSizeLimits()
    {
        ...
        GetBrushPanel().SizeSlider.Maximum = max;
    ...
    private void SetActiveToolSize(double size)
    {
        ...
        if (preset?.Kind.IsBrushFamily() == true)
        {
            SetBrushSizeLive(clamped);
            return;
        }

        GetBrushPanel().SizeSlider.Value = clamped;
    }
    ...
    private void SetBrushSizeLive(double size)
    {
        ...
        GetBrushPanel().StrokePreview.Brush = applied;

        GetBrushPanel().SyncingBrushUi = true;
        try
        {
            GetBrushPanel().SizeSlider.Value = Math.Clamp(size, GetBrushPanel().SizeSlider.Minimum, GetBrushPanel().SizeSlider.Maximum);
        }
        finally
        {
    ...
    {
        if (_activeToolGroup?.ActivePreset?.Kind == ToolKind.Liquify)
        {
