...
public interface ICanvasInputHost
{
    ...
    // ── Layer pick (Ctrl+Shift canvas shortcut) ──
    bool IsLayerPickDrag { get; }
    void StartLayerPickDrag(Point canvasPosition);
    void UpdateLayerPickDrag(Point canvasPosition);
    void CompleteLayerPickDrag(Point viewportPosition, Point canvasPosition);
    void CancelLayerPickDrag();
...
public sealed class CanvasInputRouter
{
        ...
        KeyModifiers? currentModifiers = null)
    {
        ...
        // Ctrl+Shift: drag to sample layers, then show picker for matches only
        if (IsLayerPickGestureReady() && isPointerActivation)
        {
            BeginLayerPickDrag(viewportPos, pointerId, eventArgs);
            return;
        }
    ...
    public void HandlePointerMove(PointerEventArgs e, Point viewportPos)
    {
        ...
        {
            _host.UpdateLayerPickDrag(_host.GetCanvasPosition(e));
            e.Handled = true;
            return;
        }
    ...
    public void HandlePointerRelease(object? eventArgs)
    {
        ...
        if (_runningAction == CanvasAction.LayerPick)
        {
            if (eventArgs is PointerReleasedEventArgs e)
                _host.CompleteLayerPickDrag(_host.GetViewportPosition(e), _host.GetCanvasPosition(e));
        }
    ...
    private void SyncModifierMask(KeyModifiers pointerModifiers)
    {
        // Pointer packets (especially pen) often omit keyboard modifiers already
        // tracked from KeyDown/KeyUp. Never let a pointer event clear them.
        var merged = _heldModifiers | pointerModifiers;
        if (merged == _heldModifiers)
            return;

        _heldModifiers = merged;
        _host.SetCanvasModifiers(merged);
        ReevaluateModifierState();
    }
    ...
    private bool IsLayerPickGestureReady() =>
        _readyAction == CanvasAction.LayerPick
        || (_activeModifierAction == ModifierAction.ChangeToolTemporarily
            && _activeModifierPresetId == ToolGroupConfig.SelectLayerPresetId);

    private void BeginLayerPickDrag(Point viewportPos, long pointerId, object? eventArgs)
    {
        if (eventArgs is PointerPressedEventArgs e)
        {
            _host.StartLayerPickDrag(_host.GetCanvasPosition(e));
            _host.CapturePointer(e.Pointer);
            e.Handled = true;
        }

        EnterRunning(CanvasAction.LayerPick, pointerId, isTemporaryPreset: false);
    }
    ...
    private void BeginBrushSizeGesture(Point viewportPos, long pointerId, PointerPressedEventArgs e)
    {
        ...
        else
        {
            ...
            // increases size and leftward decreases (matches convention).
            _brushSizeGestureCenterCanvasPoint = new Point(
    ...
    private void ReevaluateModifierState()
    {
        ...
        var toolKind = policy.ModifierToolKind;

        ModifierKeyAssignment? assignment = null;

        foreach (var heldKey in _heldBaseKeys)
        {
            var a = App.ModifierKeys.Resolve(toolKind, heldKey, _heldModifiers);
            if (a != null && a.Key.HasValue) { assignment = a; break; }
        }

        if (assignment == null && _heldBaseKeys.Count == 0)
            assignment = App.ModifierKeys.Resolve(toolKind, null, _heldModifiers);

        // rule: modifier changes do NOT interrupt a running transaction.
        if (_state == RouterState.Running)
            return;
    ...
    {
        if (_state is not RouterState.Idle and not RouterState.Ready) return CanvasAction.None;
