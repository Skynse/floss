...
public readonly struct CanvasInputPolicy
{
    ...
    public (int Input, int Output) ModifierToolTypes { get; init; }

    public static CanvasInputPolicy ForActivePreset(int inputProcess, int outputProcess) => new()
    {
        IsTransientEdit = false,
        ModifierToolTypes = (inputProcess, outputProcess),
    };

    public static CanvasInputPolicy TransientEdit { get; } = new()
    {
        IsTransientEdit = true,
        // No tool-specific table — resolve against general assignments only (Ctrl = none, Space = pan).
        ModifierToolTypes = (0, 0),
    };
