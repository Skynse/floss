...
public static class PsdExporter
{
    public static void Export(Stream stream, DrawingDocument document)
    {
        ...
        var layers = BuildPsdLayerStack(document, width, height);
    ...
    private static List<ExportLayer> BuildPsdLayerStack(DrawingDocument document, int width, int height)
    {
        var paperFill = document.IsPaperBackgroundVisible ? document.PaperColor : (Color?)null;
        var result = new List<ExportLayer>();
        foreach (var root in document.Layers.Where(layer => layer.Parent == null))
            AppendPsdLayer(result, root, width, height, paperFill);
        return result;
    }

    private static void AppendPsdLayer(List<ExportLayer> result, DrawingLayer layer, int width, int height, Color? paperFill)
    {
        if (layer.IsGroup)
        {
            result.Add(ExportLayer.GroupDivider(layer, ExportBounds(layer, width, height)));
            foreach (var child in layer.Children)
                AppendPsdLayer(result, child, width, height, paperFill);
            result.Add(ExportLayer.GroupHeader(layer, ExportBounds(layer, width, height)));
            return;
        }

        var bounds = ExportBounds(layer, width, height);
        if (layer.IsPaper && paperFill is { } fill)
            result.Add(ExportLayer.PaperLayer(layer, bounds, fill));
        else
            result.Add(ExportLayer.PaintLayer(layer, bounds));
    }
    ...
    private static byte[] CompressChannelRle(ExportLayer exportLayer, int width, int height, int channel)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            {
                byte b, g, r, a;
                if (exportLayer.PaperFill is { } paper)
                {
                    b = paper.B;
                    g = paper.G;
                    r = paper.R;
                ...
                else
                {
                    ...
                    layer.Pixels.GetPixel(localX, localY, out b, out g, out r, out a);
                }
    ...
    private static void WriteCompositeImageData(PsdBinaryWriter w, DrawingDocument document, int width, int height)
    {
        ...
        var pixels = compositor.CompositeToBgra(document.Layers, width, height, PaperColorToBgra(document));
    ...
    private static uint PaperColorToBgra(DrawingDocument document)
    {
        if (!document.IsPaperBackgroundVisible)
            return 0;

        var c = document.PaperColor;
        return (uint)(c.B | (c.G << 8) | (c.R << 16) | (c.A << 24));
    }

    private sealed record ExportLayer(DrawingLayer Layer, PixelRegion Bounds, int? SectionType, Color? PaperFill = null)
    {
        public static ExportLayer PaintLayer(DrawingLayer layer, PixelRegion bounds) => new(layer, bounds, null);
        public static ExportLayer PaperLayer(DrawingLayer layer, PixelRegion bounds, Color fill) => new(layer, bounds, null, fill);
