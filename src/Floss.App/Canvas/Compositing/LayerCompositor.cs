...
/// <summary>
/// GlCanvasImpl: all tiles stored in one contiguous flat buffer
...
public sealed class LayerCompositor : IDisposable
{
    ...
    private const int StrokeSuspendTileBudget = 256;
    ...
    // PixmapGrid max cell dimension (32000 for Qt's max pixmap)
    private const int MaxCellDim = 4096 * 4; // 16384 — fits in most GPU texture limits

    // ── Cells: one SKBitmap per cell (PixmapGrid) ───────────────
    ...
    // ── Per-tile composite scratch (transient tile per thread) ──
    ...
    public void SetSize(int w, int h)
    {
        ...
        // GlCanvasImpl.resizeImpl: zero the display buffer on resize.
    ...
    {
        for (var i = 0; i < _cellRevision.Length; i++)
            _cellRevision[i]++;
    }
    ...
    // ═══ DrawTiles — one SkiaTileDrawOp per cell ═══════════════════
    ...
    private unsafe void CopyTileToCell(int ti)
    {
        ...
        _cellRevision[ci]++;
        DropCellDisplayImage(ci);
    }

    private void DropCellDisplayImage(int ci)
    {
        if ((uint)ci >= (uint)_cellImages.Length) return;
        var img = _cellImages[ci];
        if (img == null) return;
        _delayedDispose.Enqueue((img, 8));
        _cellImages[ci] = null;
        if ((uint)ci < (uint)_cellImageRevision.Length)
            _cellImageRevision[ci] = -1;
    }
        ...
        int? layerIndex, bool metadataOnly = false, PixelRegion? viewportClip = null)
    {
        lock (CompositeGate)
        {
            ...
            {
                // track the FULL dirty region for eventual compositing,
    ...
    private void EnsureTileScratchArray()
    {
        ...
        // GlCanvasImpl.resizeImpl: zero the flat buffer, don't preserve
        ...
        out StrokeSplitPlan plan)
    {
        ...
        for (var ancestor = paintLayer.Parent; ancestor != null; ancestor = ancestor.Parent)
        {
            if (ancestor.IsGroup && ancestor.BlendMode != BlendMode.PassThrough)
                return false;
        }
        ...
        for (var gi = 0; gi < rootStack.Count; gi++)
        {
            ...
            if (directChild == null || directChild.IsGroup) return false;
