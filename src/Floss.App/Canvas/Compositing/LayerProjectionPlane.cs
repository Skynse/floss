...
/// <summary>
/// -style layer merge: per-tile recursion into groups, no cached
/// projection buffers. Groups are flattened into a temp buffer, then merged
/// with the group's blend mode and opacity ("isolated" path).
...
internal sealed class LayerProjectionPlane
{
        ...
        for (var i = 0; i < stack.Count; i++)
        {
            ...
            if (item.IsClipped && item.BaseLayerIndex >= 0)
            {
                ...
                try
                {
                    ...
                    fixed (byte* tp = temp)
                    {
                        // base must be composited into temp with Normal
    ...
    /// Matches group children as clipping layers flatten into the
        ...
        DrawingLayer group, PixelRegion clip, int originX, int originY)
    {
        ...
        else
        {
            ...
            // then merge onto dst using alpha-preserving blend (
            // with → alpha-preserving).
            var tempLen = clip.Width * clip.Height * 4;
            var temp = ArrayPool<byte>.Shared.Rent(tempLen);
            try
            {
        ...
        PixelRegion clip, int originX, int originY)
    {
        ...
        // pass-through → recurse with accumulated opacity
        if (group.BlendMode == BlendMode.PassThrough)
        {
        ...
        // isolated group: flatten children at full opacity into
