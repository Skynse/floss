...
public sealed class DrawingCanvas : Control, IDisposable
{
    ...
    private Point _assistantSnapStrokeBegin;
    ...
    private const double LayerPickDragThreshold = 4.0;
    ...
    public DrawingCanvas()
    {
        ...
        _ctx = new ToolContext(_document)
        {
            ...
            ActivatePresetById = presetId => ActivatePresetById?.Invoke(presetId),
        };
        ...
        _document.Assistants.Changed += (_, _) =>
        {
    ...
    /// <summary>
    /// Invalidate compositor tiles for <paramref name="region"/> without viewport clipping.
    /// Use after filter preview revert so off-screen tiles are not left stale.
    /// </summary>
    internal void InvalidateCompositorUnclipped(PixelRegion? region = null, int? layerIndex = null)
    {
        _projectionScheduler.ApplyPending(_compositor);
        _compositor.Invalidate(region, _document.Layers, layerIndex, viewportClip: null);
        InvalidateVisual();
    }
    ...
    public bool HasSavedBrushSettings(ToolKind kind) => _toolController.HasSavedSettings(kind);
    public bool HasDocument => _document.Layers.Count > 0;

    /// <summary>Plugin and feature overlays drawn above the document, below the tool cursor.</summary>
    public ICanvasOverlayRegistry? OverlayRegistry { get; set; }

    /// <summary>True while a paint/transform stroke is updating the compositor.</summary>
    public bool IsStrokeInProgress => _compositor.StrokeSuspendActive;
    ...
    public bool TryDeleteSelectedAssistant()
    {
        if (!IsAssistantOrObjectTool())
            return false;

        var id = _document.Assistants.SelectedId;
        if (id == null)
            return false;

        var before = _document.Assistants.CaptureSnapshot();
        if (!_document.Assistants.Remove(id))
            return false;

        _document.CommitAssistantsChange(before);
        InvalidateVisual();
        return true;
    }

    private bool IsAssistantOrObjectTool()
        => _toolController.ActiveTool is AssistantTool
            || _toolController.ActiveTool is Processes.CompositeTool { Output: ObjectOutput };

    private bool ShouldSnapPointerToAssistants()
    {
        if (_toolController.HasViewportNavOverlay)
            return false;
        if (_toolController.ActiveTool is AssistantTool)
            return false;
        if (_ctx.ActivePreset is not { } preset)
            return false;

        return preset.Kind.IsBrushFamily() || preset.Kind == ToolKind.Liquify;
    }
    ...
    public void HandlePointerInput(ToolInputEventKind kind, PointerPoint point)
    {
        ...
        if (ShouldSnapPointerToAssistants())
        {
            var docPoint = new Point(docX, docY);
            if (kind == ToolInputEventKind.Down)
                _assistantSnapStrokeBegin = docPoint;
            docPoint = AssistantSnap.AdjustPosition(_document.Assistants, docPoint, _assistantSnapStrokeBegin);
            docX = docPoint.X;
            docY = docPoint.Y;
        }
        ...
        if (OverlayRegistry?.TryHandlePointer(kind, point, _ctx.CurrentModifiers, _document.Width, _document.Height, w, h) == true)
        {
            InvalidateVisual();
            return;
        }
    ...
    public CanvasInputPolicy InputPolicy
    {
        get
        {
            ...
            return _ctx.ActivePreset is { } preset
                ? CanvasInputPolicy.ForActivePreset(preset.Kind)
                : CanvasInputPolicy.ForActivePreset(default);
        }
    }
    ...
    public bool JumpToHistoryIndex(int index)
    {
        EnsureStrokeOutputIdle();
        if (_toolController.HasPendingOperation && !(_toolController.ActiveTool is TransformTool))
            _toolController.Cancel();
        EnsureStrokeOutputIdle();
        if (IsStrokeOutputPending())
            return false;

        _document.JumpToHistoryIndex(index);
        InvalidateAfterHistoryReplay();
        InvalidateVisual();
        return true;
    }
    ...
    public void ToggleLayerRulersVisibility(int index)
    {
        if (index < 0 || index >= _document.Layers.Count)
            return;

        var layer = _document.Layers[index];
        if (layer.RulerSet is not { HasRulers: true })
            return;

        layer.RulerSet.RulersVisible = !layer.RulerSet.RulersVisible;
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (HasAnyLayerContent(_document.Layers))
        {
            ...
            // UI draws tiles directly from cache (no all-or-nothing
        ...
        var assistantPreview = _toolController.ActiveTool is AssistantTool assistantTool
            ...
            : null;
        AssistantsRenderer.Render(
            context,
            _document.Assistants,
            _document.Width,
            _document.Height,
        ...
        OverlayRegistry?.Render(context, this, CanvasZoom);
    ...
    private void DrawToolCursorAt(DrawingContext context, Point pos, double radiusScale)
    {
        ...
        if (_toolController.IsAlternateActive
            || _ctx.ActivePreset?.Kind == ToolKind.Eyedropper)
        {
            var swatchR = 10.0;
    ...
    {
        var cfg = App.Config;
        return _ctx.ActivePreset?.Kind switch
        {
            ToolKind.Pen => cfg.PenCursorMode,
            ToolKind.Eraser => cfg.EraserCursorMode,
            ToolKind.Smudge => cfg.SmudgeCursorMode,
            _ => cfg.BrushCursorMode
        };
    }

    private double ActiveToolCursorSize()
        => _ctx.ActivePreset?.Kind switch
        {
            ToolKind.Liquify => _ctx.ActivePreset.LiquifySize,
            _ => _brush.Size
        };
    ...
    private void DrawBrushShapeCursor(DrawingContext context, Point pos, double t, double radiusScale = 1.0)
    {
        ...
        if (r < 0.5) return;
        var tip = _brush.Tip;
        var useTipMask = _ctx.ActivePreset?.Kind == ToolKind.Brush;
    ...
    public void StartLayerPickDrag(Point canvasPosition)
    {
        _isLayerPickDrag = true;
        _layerPickAnchor = canvasPosition;
        _layerPickCurrent = canvasPosition;
        InvalidateVisual();
    }

    public void UpdateLayerPickDrag(Point canvasPosition)
    {
    ...
    public IReadOnlyList<int> CompleteLayerPickDrag(Point canvasPosition)
    {
        if (!_isLayerPickDrag)
        ...
        var anchorDoc = CanvasPointToDocument(_layerPickAnchor);
        var currentDoc = CanvasPointToDocument(_layerPickCurrent);

        if (dragDist < LayerPickDragThreshold)
        {
            return LayerPickQueries.FindLayersAtPoint(
                _document,
                (int)anchorDoc.X,
                (int)anchorDoc.Y);
        }

        var rx = (int)Math.Min(anchorDoc.X, currentDoc.X);
        var ry = (int)Math.Min(anchorDoc.Y, currentDoc.Y);
        var rw = (int)Math.Abs(currentDoc.X - anchorDoc.X) + 1;
        var rh = (int)Math.Abs(currentDoc.Y - anchorDoc.Y) + 1;
        return LayerPickQueries.FindLayersInRect(_document, rx, ry, rw, rh);
    }

    private Point CanvasPointToDocument(Point canvasPoint)
    {
        var w = Math.Max(1, Bounds.Width);
        var h = Math.Max(1, Bounds.Height);
        return new Point(
            canvasPoint.X / w * _document.Width,
            canvasPoint.Y / h * _document.Height);
    }

    public void CancelLayerPickDrag()
    {
        if (!_isLayerPickDrag) return;
        _isLayerPickDrag = false;
        InvalidateVisual();
