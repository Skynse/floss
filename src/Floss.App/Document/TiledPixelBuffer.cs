...
public sealed class TiledPixelBuffer : IDisposable
{
    ...
    private readonly ReaderWriterLockSlim _pixelLock = new(LockRecursionPolicy.SupportsRecursion);
    ...
    // Marks an in-progress brush stroke. BrushEngine uses this to pick stroke-mask
    // compositing; tile bytes still use the reader/writer lock so the compositor
    // never reads partially written dab pixels (which breaks blend-mode preview).
