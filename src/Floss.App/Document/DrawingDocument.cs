...
public sealed class DrawingDocument : IDisposable
{
    ...
    private readonly List<DocumentHistoryEntry> _timeline = [];
    ...
    private string? _pendingHistoryLabel;
    private int _historyJumpDepth;
    ...
    public DrawingDocument(int width = 2048, int height = 2048)
    {
        ...
        ActiveLayerIndex = -1;
        Assistants = new DocumentAssistants(this);
    ...
    /// <summary>Active layer index changed (selection only — not structure).</summary>
    ...
    public DocumentAssistants Assistants { get; }
    public int CommittedStrokeCount { get; private set; }
    public bool CanUndo => _undo.Count > 0;
    public bool CanRedo => _redo.Count > 0;
    public IReadOnlyList<DocumentHistoryEntry> HistoryEntries => _timeline;
    public int HistoryIndex => _timelineIndex;

    /// <summary>Label for the next <see cref="PushHistoryState"/> call (e.g. active tool preset name).</summary>
    public void SetPendingHistoryLabel(string? label) => _pendingHistoryLabel = label;
    ...
    public bool CanPaintActiveLayer => ActiveLayer is { IsLocked: false, IsVisible: true, IsGroup: false, IsObjectLayer: false };
    public bool CanModifyActiveLayer => ActiveLayer is { IsLocked: false, IsGroup: false, IsObjectLayer: false };
    public bool IsDirty => _currentStateId != _savedStateId;

    /// <summary>Monotonic revision for document pixels/metadata affecting composited output (overview cache, etc.).</summary>
    public long VisualRevision => _currentStateId;
    ...
    public void MarkAsSaved()
    {
        ...
        if (_timelineIndex >= 0 && _timelineIndex < _timeline.Count)
            _timeline[_timelineIndex] = _timeline[_timelineIndex] with { IsSaved = true };
        DirtyStateChanged?.Invoke(this, EventArgs.Empty);
    }

    public void JumpToHistoryIndex(int index)
    {
        if (index < 0 || index >= _timeline.Count || index == _timelineIndex)
            return;

        _historyJumpDepth++;
        try
        {
            while (_timelineIndex > index)
                Undo(suppressNotification: true);

            while (_timelineIndex < index)
                Redo(suppressNotification: true);
        }
        finally
        {
            _historyJumpDepth--;
            if (_historyJumpDepth == 0)
            {
    ...
    private void ResetHistoryTimeline(string label = "Document")
    {
        _timeline.Clear();
        _timeline.Add(new DocumentHistoryEntry(0, label, IsSaved: false));
        _timelineIndex = 0;
    }
    ...
    private void PushHistoryState(IHistoryState state)
    {
        ...
        if (_timelineIndex < _timeline.Count - 1)
            _timeline.RemoveRange(_timelineIndex + 1, _timeline.Count - _timelineIndex - 1);

        var label = _pendingHistoryLabel ?? DescribeHistoryState(state);
        _pendingHistoryLabel = null;
        _timeline.Add(new DocumentHistoryEntry(_currentStateId, label, IsSaved: false));
    ...
    public void ResizeCanvas(int newW, int newH, int contentOffsetX, int contentOffsetY)
    {
        ...
        foreach (var layer in _layers)
        {
            var nl = new DrawingLayer(layer.Name, newW, newH)
            {
                ...
                ObjectContent = layer.ObjectContent?.Clone(),
            };
            if (!layer.IsGroup)
            {
                if (layer.IsPaper)
    ...
    public void CommitAssistantsChange(AssistantsSnapshot before)
    {
        var after = Assistants.CaptureSnapshot();
        if (AssistantsSnapshotsEqual(before, after)) return;
        PushHistoryState(new AssistantsHistoryState(before, after));
    }
    ...
    public DrawingLayer ResolveRulerHostLayer()
    {
        if (ActiveLayer is { IsPaper: false, IsGroup: false, IsObjectLayer: false } active)
            return active;

        var fallback = _layers.FirstOrDefault(l => !l.IsPaper && !l.IsGroup && !l.IsObjectLayer);
        if (fallback != null)
            return fallback;

        return AddRulerHostLayer();
    }

    public DrawingLayer AddRulerHostLayer()
    {
        BeginDocumentMutation();
        var layer = new DrawingLayer($"Layer {_layers.Count + 1}", Width, Height);
        _layers.Add(layer);
    ...
    // --- Undo / Redo Execution ---
    public void Undo(bool suppressNotification = false)
    {
        ...
        LastHistoryAffectsVisual = state.AffectsVisual;
        LastHistoryVisualDirtyRegion = state.VisualDirtyRegion;
        _timelineIndex = Math.Max(0, _timelineIndex - 1);
        if (!suppressNotification && _historyJumpDepth == 0)
        {
            HistoryChanged?.Invoke(this, EventArgs.Empty);
            DirtyStateChanged?.Invoke(this, EventArgs.Empty);
        }
    ...
    public void Redo(bool suppressNotification = false)
    {
        ...
        LastHistoryAffectsVisual = state.AffectsVisual;
        LastHistoryVisualDirtyRegion = state.VisualDirtyRegion;
        _timelineIndex = Math.Min(_timeline.Count - 1, _timelineIndex + 1);
        if (!suppressNotification && _historyJumpDepth == 0)
        {
            HistoryChanged?.Invoke(this, EventArgs.Empty);
            DirtyStateChanged?.Invoke(this, EventArgs.Empty);
    ...
    private static string DescribeHistoryState(IHistoryState state) => state switch
    {
        LayerTileHistoryState { MaskMutation: true } => "Mask paint",
        LayerTileHistoryState => "Paint",
        LayerRegionHistoryState => "Paint",
        SelectionHistoryState => "Selection",
        AssistantsHistoryState => "Assistants",
        InsertLayerHistoryState => "New layer",
        RemoveLayerHistoryState => "Delete layer",
        MoveLayerHistoryState => "Move layer",
        LayerOffsetHistoryState => "Move layer",
        LayerMaskTilesHistoryState => "Layer mask",
        LayerApplyMaskHistoryState => "Apply mask",
        AdjustmentParamsHistoryState => "Adjustment",
        SnapshotHistoryState => "Document state",
        CompositeHistoryState => "Edit",
        LayerPropertyHistoryState<bool> => "Layer property",
        LayerPropertyHistoryState<string> => "Rename layer",
        LayerPropertyHistoryState<BlendMode> => "Blend mode",
        LayerPropertyHistoryState<Avalonia.Media.Color?> => "Layer color",
        DocumentPropertyHistoryState<int> => "Document DPI",
        DocumentPropertyHistoryState<Avalonia.Media.Color> => "Paper color",
        LayerPropertyHistoryState<double> => "Layer opacity",
        _ => "Edit"
    };
    ...
    internal static DrawingLayer CloneLayerTree(DrawingLayer source, string? nameOverride = null)
    {
        var copy = new DrawingLayer(nameOverride ?? source.Name, source.Width, source.Height)
        {
            ...
            RulerSet = source.RulerSet?.Clone(),
            IsObjectLayer = source.IsObjectLayer,
            ObjectContent = source.ObjectContent?.Clone(),
        };
    ...
    private static bool AssistantsSnapshotsEqual(AssistantsSnapshot a, AssistantsSnapshot b)
    {
        if (a.SelectedId != b.SelectedId || a.LayerSets.Count != b.LayerSets.Count)
            return false;

        for (var i = 0; i < a.LayerSets.Count; i++)
        {
            var xSet = a.LayerSets[i];
            var ySet = b.LayerSets[i];
            if (xSet.LayerIndex != ySet.LayerIndex
                || xSet.Set.ShowScope != ySet.Set.ShowScope
                || xSet.Set.LinkToLayer != ySet.Set.LinkToLayer
                || xSet.Set.RulersVisible != ySet.Set.RulersVisible
                || xSet.Set.Rulers.Count != ySet.Set.Rulers.Count)
                return false;

            for (var j = 0; j < xSet.Set.Rulers.Count; j++)
            {
                var x = xSet.Set.Rulers[j];
                var y = ySet.Set.Rulers[j];
                if (x.Id != y.Id || x.TypeId != y.TypeId || x.IsVisible != y.IsVisible || x.SnapEnabled != y.SnapEnabled
                    || x.PerspectiveMode != y.PerspectiveMode || x.FisheyeEnabled != y.FisheyeEnabled
                    || x.FovDegrees != y.FovDegrees || x.GridSubdivisions != y.GridSubdivisions
                    || x.HandleA != y.HandleA || x.HandleB != y.HandleB || x.HandleC != y.HandleC || x.HandleD != y.HandleD)
                    return false;
            }
        }
    ...
    private sealed record AssistantsHistoryState(AssistantsSnapshot Before, AssistantsSnapshot After) : IHistoryState
    {
        public bool AffectsVisual => false;
        public PixelRegion VisualDirtyRegion => PixelRegion.Empty;

        public IHistoryState CaptureRedo(DrawingDocument document)
            => new AssistantsHistoryState(After, Before);

        public void Restore(DrawingDocument document)
            => document.Assistants.RestoreSnapshot(Before);
    }
