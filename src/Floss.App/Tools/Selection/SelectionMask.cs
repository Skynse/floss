...
public sealed class SelectionMask
{
    ...
    public void SetFromPolygon(IReadOnlyList<SKPoint> points, SelectOp op = SelectOp.Replace)
    {
        ...
        // approach: rasterize the path into an Alpha8 bitmap in one
    ...
    public void SetFromFloodFill(
        ...
        int areaScaling = 0)
        => SetFromFloodFillLayerLocal(
            pixels, srcX, srcY, offsetX, offsetY, pixels.Width, pixels.Height,
            tolerance, op, contiguousFill, areaScaling);

    /// <summary>
    /// Flood-fill in layer-local coordinates, then map hits into the document selection mask.
    /// Prevents offset layers from connecting through empty document space.
    /// </summary>
    public void SetFromFloodFillLayerLocal(
        TiledPixelBuffer pixels,
        int localX,
        int localY,
        int offsetX,
    ...
    {
        EnsureMaskExists();
        if ((uint)localX >= (uint)layerW || (uint)localY >= (uint)layerH)
            return;

        pixels.GetPixel(localX, localY, out byte refB, out byte refG, out byte refR, out byte refA);
        int threshold = ColorDifference.Tolerance01ToThreshold(tolerance);

        var next = CreateBaseMask(op);
        var simCache = new Dictionary<int, bool>(1024);

        bool Similar(int lx, int ly)
        {
            if ((uint)lx >= (uint)layerW || (uint)ly >= (uint)layerH)
                return false;

            pixels.GetPixel(lx, ly, out byte b, out byte g, out byte r, out byte a);
        ...
        RunFloodFillIntoLayerLocalMask(next, localX, localY, layerW, layerH, offsetX, offsetY, op, Similar, contiguousFill);
        ApplyAreaScalingToMask(next, areaScaling);
        CommitMask(next);
        FinalizeMaskSelection();
    }
    ...
    private void RunFloodFillIntoLayerLocalMask(
        byte[] next,
        int startLocalX,
        int startLocalY,
        int layerW,
        ...
        bool contiguousFill)
    {
        ...
        if (contiguousFill)
        {
            _visit.BeginPass(layerW * layerH);
            FloodFillScanline.FillContiguous(layerW, layerH, startLocalX, startLocalY, similar,
                _visit.Stamp, _visit.Epoch, OnPixel);
            if (op == SelectOp.Intersect)
                ClearUnvisitedLayerLocalIntersect(next, offsetX, offsetY, layerW, layerH);
        }
        else if (op == SelectOp.Intersect)
        {
            for (int ly = 0; ly < layerH; ly++)
            {
                for (int lx = 0; lx < layerW; lx++)
                {
                    ...
                    if ((uint)docX >= (uint)_docW || (uint)docY >= (uint)_docH)
                        continue;
                    Apply(next, docX, docY, SelectOp.Intersect, true);
                }
            }
        }
        else
        {
            FloodFillNonContiguous.FillInBounds(0, 0, layerW - 1, layerH - 1, similar, OnPixel);
        }
    }
    ...
    private void ClearUnvisitedLayerLocalIntersect(byte[] next, int offsetX, int offsetY, int layerW, int layerH)
    {
        if (_mask == null || _maskBounds is null) return;

        var layerDocRect = new SKRectI(
            offsetX,
            offsetY,
            Math.Min(_docW, offsetX + layerW),
            Math.Min(_docH, offsetY + layerH));
        var iter = IterationBoundsForIntersect(layerDocRect);
        for (int py = iter.Top; py < iter.Bottom; py++)
        {
            for (int px = iter.Left; px < iter.Right; px++)
            {
                ...
                if ((uint)lx >= (uint)layerW || (uint)ly >= (uint)layerH)
                    continue;
                int localIdx = ly * layerW + lx;
                if (_visit.Stamp[localIdx] == _visit.Epoch)
                    continue;
                Apply(next, px, py, SelectOp.Intersect, false);
            }
        }
