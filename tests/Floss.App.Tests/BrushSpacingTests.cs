...
public class BrushSpacingTests
{
    ...
    [Fact]
    public void BrushEngine_ShortStabilizerSegments_AccumulateSpacingLikeOneLongSegment()
    {
        using var engine = new BrushEngine();
        using var layer = new DrawingLayer("Layer", 512, 512);
        var brush = new BrushPreset("Spaced", 48, 1, 0.75, 0.25, Colors.Black, 0)
        {
            AutoSpacingActive = false,
            Tip = new ProceduralBrushTip(BrushTipShape.Circle),
            Dynamics = new BrushDynamics()
        ...
        var start = Sample(40, 256, 0);

        // Simulate slow stroke: many short stabilizer segments along the same distance.
        var many = new List<CanvasInputSample> { start };
        for (var i = 1; i <= 40; i++)
            many.Add(Sample(40 + i * 4, 256, i * 400));
        engine.BeginStroke(brush, many[0]);
        engine.RasterizeSegments(layer, brush, many, 1, many.Count - 1);
        var manySegCount = engine.LastStats.StampCount;

        var one = new List<CanvasInputSample> { start, Sample(200, 256, 16_000) };
        engine.BeginStroke(brush, one[0]);
        engine.RasterizeSegments(layer, brush, one, 1, 1);
        var oneSegCount = engine.LastStats.StampCount;

        TestAssertions.True(
            Math.Abs(manySegCount - oneSegCount) <= 1,
            $"Short segments ({manySegCount} stamps) should match one long segment ({oneSegCount} stamps) for the same spacing.");
    }
