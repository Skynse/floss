...
public class ClipImporterTests
{
    ...
    public void ParseChunkBlocks_EmptyBlocks_ReturnsArray()
    {
        ...
        using var stream = File.OpenRead(
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "anat2.clip"));
    ...
    public void Importer_LoadsRealClipFile()
    {
        var path = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "anat2.clip");
        if (!File.Exists(path)) return;
    ...
    public void MapBlendMode_MapsCorrectly(int clipComposite, string expectedFlossMode)
    {
        var method = typeof(Floss.App.Clip.ClipImporter)
            .GetMethod("MapBlendMode",
                System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic)!;
        var result = (BlendMode)method.Invoke(null, [clipComposite, false])!;
