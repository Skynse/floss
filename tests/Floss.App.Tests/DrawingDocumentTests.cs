...
public class DrawingDocumentTests
{
    ...
    [Fact]
    public void LayerOpacityScrub_OneHistoryEntryPerGesture()
    {
        var document = new DrawingDocument(4, 4);
        document.AddLayer();
        document.ActiveLayer!.Opacity = 1.0;

        var undoBefore = document.HistoryIndex;

        document.BeginActiveLayerOpacityScrub();
        document.PreviewActiveLayerOpacity(0.8);
        document.PreviewActiveLayerOpacity(0.5);
        document.PreviewActiveLayerOpacity(0.3);
        TestAssertions.Equal(undoBefore, document.HistoryIndex);

        document.CommitActiveLayerOpacityScrub();
        TestAssertions.Equal(0.3, document.ActiveLayer.Opacity);
        TestAssertions.Equal(undoBefore + 1, document.HistoryIndex);
    ...
    public void MergeSelectedLayers_CombinesMultipleLayers()
    {
        ...
        TestAssertions.Equal(1, document.Layers.Count);
        // Merged layer inherits the topmost selected layer's name (stack index 2).
        TestAssertions.Equal("Layer 3", document.ActiveLayer!.Name);
        TestAssertions.Equal(0, document.ActiveLayerIndex);
    }
