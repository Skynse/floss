...
public class PsdExporterTests
{
    ...
    [Fact]
    public void Export_IncludesPaperLayerWithDocumentColor()
    {
        var document = new DrawingDocument(2, 2);
        document.AddBackgroundLayer();
        document.SetPaperColor(Avalonia.Media.Color.FromArgb(255, 200, 100, 50)); // A,R,G,B
        document.AddLayer();
        document.ActiveLayer!.Pixels.SetPixel(1, 1, 50, 100, 200, 255); // B,G,R,A
        ...
        var paper = psd.Layers[0] as PsdLayerNode
            ?? throw new InvalidOperationException("Expected exported PSD to include the paper layer.");
        TestAssertions.Equal("Paper", paper.Name);
        TestAssertions.True(paper.IsVisible);
        TestAssertions.Equal(2, paper.Width);
        TestAssertions.Equal(2, paper.Height);
        TestAssertions.True(paper.Bgra != null, "Expected paper layer pixels.");
        TestAssertions.SequenceEqual(new byte[] { 50, 100, 200, 255 }, paper.Bgra!.Take(4));

        var ink = psd.Layers[1] as PsdLayerNode
            ?? throw new InvalidOperationException("Expected exported PSD to include the paint layer.");
        TestAssertions.SequenceEqual(new byte[] { 50, 100, 200, 255 }, ink.Bgra!.Skip((1 * ink.Width + 1) * 4).Take(4));
    }

    [Fact]
    public void Export_CompositeIncludesPaperBackdrop()
    {
        var document = new DrawingDocument(2, 2);
        document.AddBackgroundLayer();
        document.SetPaperColor(Avalonia.Media.Color.FromArgb(255, 10, 20, 30)); // A,R,G,B

        var data = ExportSamplePsd(document);
        var compositeSize = 2 + 2 * 2 * 4;
        var composite = data.AsSpan(data.Length - compositeSize);
        TestAssertions.Equal((ushort)0, BinaryPrimitives.ReadUInt16BigEndian(composite));
        var planes = composite.Slice(2);
        TestAssertions.True(planes[..4].ToArray().All(b => b == 10), "Expected R plane.");
        TestAssertions.True(planes.Slice(4, 4).ToArray().All(b => b == 20), "Expected G plane.");
        TestAssertions.True(planes.Slice(8, 4).ToArray().All(b => b == 30), "Expected B plane.");
        TestAssertions.True(planes.Slice(12, 4).ToArray().All(b => b == 255), "Expected A plane.");
    }
    ...
    private static byte[] ExportSamplePsd(DrawingDocument? document = null)
    {
        document ??= CreateInkSampleDocument();
    ...
    private static DrawingDocument CreateInkSampleDocument()
    {
        var document = new DrawingDocument(2, 2);
        document.AddLayer();
