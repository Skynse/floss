...
public class KraImporterTests
{
    ...
    [Fact]
    public void Load_MapsChannelFlagsInheritAlphaToClipping()
    {
        var kra = BuildInheritAlphaKra();
        using var stream = new MemoryStream(kra);

        var document = KraImporter.Load(stream);

        TestAssertions.Equal(2, document.Layers.Count);
        TestAssertions.False(document.Layers[0].IsClipping);
        TestAssertions.True(document.Layers[1].IsClipping);
        TestAssertions.True(document.Layers[1].IsAlphaLocked);
    }

    [Fact]
    public void Load_ImportsGroupHierarchyInArchiveOrder()
    {
    ...
    private static byte[] BuildInheritAlphaKra()
    {
        using var zipStream = new MemoryStream();
        using (var archive = new ZipArchive(zipStream, ZipArchiveMode.Create, leaveOpen: true))
        {
            ...
            WriteTextEntry(archive, "maindoc.xml", """
                ...
                <DOC xmlns="http://www.calligra.org/DTD/krita" syntaxVersion="2">
                  <IMAGE mime="application/x-kra" name="Sample" width="64" height="64" colorspacename="RGBA">
                    <layers>
                      <layer nodetype="paintlayer" name="Clip" filename="clip" compositeop="normal"
                             channelflags="1110" channellockflags="1110" opacity="255" visible="1" x="0" y="0" colorspacename="RGBA"/>
                      <layer nodetype="paintlayer" name="Base" filename="base" compositeop="normal"
                             opacity="255" visible="1" x="0" y="0" colorspacename="RGBA"/>
                    </layers>
                  </IMAGE>
                </DOC>
                """);

            WriteBinaryEntry(archive, "Sample/layers/base", BuildRawTileLayerFile(10, 20, 30, 255));
            WriteBinaryEntry(archive, "Sample/layers/clip", BuildRawTileLayerFile(40, 50, 60, 200));
        }
