...
public class CanvasInputRouterTests
{
    ...
    private sealed class MockHost : ICanvasInputHost
    {
        ...
        public Point? LayerPickCanvasPosition { get; set; }
        public Point? LayerPickerViewportPosition { get; set; }
        ...
        public CanvasInputPolicy InputPolicy { get; set; } =
            CanvasInputPolicy.ForActivePreset(ToolKind.Brush);
        ...
        {
            IsLayerPickDrag = true;
            LayerPickCanvasPosition = canvasPosition;
            Operations.Add("StartLayerPickDrag");
        }

        void ICanvasInputHost.UpdateLayerPickDrag(Point canvasPosition)
            => Operations.Add("UpdateLayerPickDrag");

        void ICanvasInputHost.CompleteLayerPickDrag(Point viewportPosition, Point canvasPosition)
        {
            IsLayerPickDrag = false;
            LayerPickerViewportPosition = viewportPosition;
            Operations.Add("CompleteLayerPickDrag");
        }

        void ICanvasInputHost.CancelLayerPickDrag()
        {
            IsLayerPickDrag = false;
            Operations.Add("CancelLayerPickDrag");
        }
    ...
    {
        var host = new MockHost { InputPolicy = CanvasInputPolicy.ForActivePreset(ToolKind.Brush) };
        var router = new CanvasInputRouter(host);
    ...
    public void AlternateInvocationFallsBackToTemporaryPresetWhenNoAlternate()
    {
        ...
        settings.ToolSpecificAssignments[ModifierKeySettings.KeyFor(ToolKind.Brush)] =
        [
    ...
    [Fact]
    public void PointerPress_DoesNotClearKeyboardModifiersFromPenPacket()
    {
        var host = new MockHost { InputPolicy = CanvasInputPolicy.ForActivePreset(ToolKind.Brush) };
        typeof(App).GetProperty(nameof(App.ModifierKeys))!.SetValue(null, ModifierKeySettings.CreateDefaults());
        var router = new CanvasInputRouter(host);
        ...
        router.HandleKeyDown(Key.LeftCtrl, KeyModifiers.Control);
        router.HandleKeyDown(Key.LeftShift, KeyModifiers.Control | KeyModifiers.Shift);
        TestAssertions.Equal(CanvasAction.LayerPick, router.ReadyAction);

        router.HandlePointerPress(
            action: CanvasAction.PrimaryTool,
            isPrimaryDown: true,
            pointerId: 1,
            viewportPos: new Point(120, 80),
            eventArgs: null,
            ctrlHeld: false,
            shiftHeld: false,
            currentModifiers: KeyModifiers.None);

        TestAssertions.Equal(CanvasAction.LayerPick, router.RunningAction);
    ...
    [Fact]
    public void LayerPickReady_StartsDragOnPress()
    {
        var host = new MockHost { InputPolicy = CanvasInputPolicy.ForActivePreset(ToolKind.Brush) };
        typeof(App).GetProperty(nameof(App.ModifierKeys))!.SetValue(null, ModifierKeySettings.CreateDefaults());
        var router = new CanvasInputRouter(host);
        ...
        router.HandlePointerPress(
            ...
            ctrlHeld: true,
            shiftHeld: true,
            currentModifiers: KeyModifiers.Control | KeyModifiers.Shift);
    ...
    public void CtrlShiftFallsThroughStaleSpecificNone()
    {
        var settings = new ModifierKeySettings
        {
            ...
            {
                [ModifierKeySettings.KeyFor(ToolKind.Brush)] =
                [
                    new ModifierKeyAssignment
                    {
                        Modifiers = KeyModifiers.Control | KeyModifiers.Shift,
        ...
        var resolved = settings.Resolve(ToolKind.Brush, null, KeyModifiers.Control | KeyModifiers.Shift);
